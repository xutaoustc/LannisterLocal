...
class AnalyticJobGeneratorHadoop3 extends AnalyticJobGenerator with Logging {
  ...
  private var _resourceManagerAddress : String = _
  ...
  override def fetchAnalyticJobs: List[AnalyticJob] = {
    ...
    info(s"[Fetching] Fetching recent finished application runs " +
      s"between last time: $start, and current time: $end")

    val succeededAppsURL = new URL(new URL("http://" + _resourceManagerAddress),
      s"/ws/v1/cluster/apps?finalStatus=SUCCEEDED" +
             s"&finishedTimeBegin=$start&finishedTimeEnd=$end")
    ...
    val failedAppsURL = new URL(new URL("http://" + _resourceManagerAddress),
      s"/ws/v1/cluster/apps?finalStatus=FAILED&state=FINISHED" +
             s"&finishedTimeBegin=$start&finishedTimeEnd=$end")
    ...
    info(s"[Fetching] Total ${appList.size} items fetched --- " +
  ...
  private def readApps(url: URL) = {
    ...
    apps.forEach(app => {
        ...
        val trackingUrl = if ( app.get("trackingUrl") != null) {
          app.get("trackingUrl").asText()
        } else null
